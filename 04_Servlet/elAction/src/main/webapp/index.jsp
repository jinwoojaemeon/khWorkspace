...
<body>
    ...
    <p>
        ...
        EL 구문을 사용하면 표현하고자하는 값을 ${name}의 형식으로 출력할 수 있다.
        EL은 기존 표현식보다 긴결하고 null처리에 안전하다.
    </p>
    ...
    <%--
        JSP구성요소
        1. 스크립팅 원소
            -JSP에서 직접 Java 코드를 작성할 수 있는 영역 
            - 예 : 스크립틀릿(<% %>), 표현식(<%= %>), 선언문(<%! %>...)
        2. 지시어 (Directive)
            - JSP 페이지의 메타데이터, 다른 JSP 포함, 라이브러리 등록 등을 설정
            - 예 : <%@ page지시어 %>, <%@ include %>, <%@ taglib %>
        3. 액션 태그(JSP Action Tag)
            - JSP에서 특정 동작을 수행하도록 지시하는 태그
            - XML 문법 기반으로 JSP 기능을 확장
            >> 표준 액션태그 : <jsp:태그명></태그명>  [jsp:] 접두어를 사용한다.
            >> 커스텀 액션태그 : 별도 라이브러리로 등록하여 사용하는 태근(JSTL 등...)
    --%>

    <h3>표준 액션 태그</h3>
    <a href="views/03_action_include.jsp">jsp:include</a> <br>
    <a href="views/04_action_forward.jsp">jsp:forward</a> <br>
    ...
    3) 2 개의 .jar 파일을 WEB-INF/lib 폴더에 추가 - copy file?
    ...
    <a href="views/05_custom_core.jsp">core</a>
    
    <h4>2. JSTL Formatting Library</h4>
    <p>숫자/날짜/시간의 출력 형식을 지정해주는 태그를 제공</p>
    <a href="views/06_custom_fmt.jsp">fmt</a>
    
    <h4>3. JSTL Function Library</h4>
    <p>el안에서 사용할 수 있는 메서드를 제공</p>
    <a href="views/07_custom_function.jsp">function</a>
</body>
</html>