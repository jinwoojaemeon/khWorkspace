-- 사용자 계정 생성
--CREATE USER C##LIFEGAMEv2 IDENTIFIED BY LIFEGAMEv2;
-- 접속 권한 및 테이블 생성 권한 부여
--GRANT CONNECT, RESOURCE TO C##LIFEGAMEv2;
-- 테이블스페이스 설정
--ALTER USER C##LIFEGAMEv2 DEFAULT TABLESPACE USERS QUOTA UNLIMITED ON USERS;

DROP TABLE CHARACTER_STOCK;
...
CREATE TABLE STOCK_LIST(
    ...
    STOCK_NAME VARCHAR2(30) UNIQUE
);

INSERT INTO STOCK_LIST VALUES(1, '삼선전자');   -- 초기가격 : 9000
INSERT INTO STOCK_LIST VALUES(2, 'KH로우닉스');  -- 초기가격 : 27000
INSERT INTO STOCK_LIST VALUES(3, '롯뚜기식품');    -- 초기가격 : 1300   
INSERT INTO STOCK_LIST VALUES(4, 'KIG넥스트');   -- 초기가격 : 800
INSERT INTO STOCK_LIST VALUES(5, '차이슬라');   -- 초기가격 : 400


CREATE TABLE CHARACTER_STOCK(
    CHARACTER_ID NUMBER,
    STOCK_ID NUMBER,
    STOCK_CNT NUMBER DEFAULT 0,
    PRICE NUMBER CONSTRAINT CHK_STOCK_PRICE_POSITIVE CHECK (PRICE>0),
    PRIMARY KEY (CHARACTER_ID, STOCK_ID),
    FOREIGN KEY (STOCK_ID) REFERENCES STOCK_LIST(STOCK_ID),
    FOREIGN KEY (CHARACTER_ID) REFERENCES LIFE_CHARACTER(CHARACTER_ID)
);
