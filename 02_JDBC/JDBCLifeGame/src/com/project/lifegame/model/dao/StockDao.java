...
public class StockDao {
    ...
    // 주식 구매
    public int buyStock(int characterId, int stockId, int count, Connection conn) {
        int result = 0;
        PreparedStatement pstmt = null;
    ...
    // 캐릭터의 주식 보유 현황 조회
    public ArrayList<Stock> getCharacterStocks(int characterId, Connection conn) {
        ResultSet rset = null;
        ArrayList<Stock> stockList = new ArrayList<>();
        PreparedStatement pstmt = null;
        
        String sql = prop.getProperty("getCharacterStocks");
        ...
        try {
            ...
            while(rset.next()) {
                ...
                stock.setStockId(rset.getInt("STOCK_ID"));
                stock.setStockName(rset.getString("STOCK_NAME"));
                stock.setStockCount(rset.getInt("STOCK_CNT"));
