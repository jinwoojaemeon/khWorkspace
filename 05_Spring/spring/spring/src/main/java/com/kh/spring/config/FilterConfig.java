...
public class FilterConfig {
    /*
        기본적으로 필터는 @Component 어노테이션으로 Bean에 등록만 해줘도 사용이 가능하지만
        좀 더 디테일한 설정을 할 때는 FilterRegustationBean이라는 객체를 사용하면 된다.
     */
    ...
    public FilterRegistrationBean<RequestTimeFilter> filterRegistrationBean(RequestTimeFilter filter){
        ...
        registration.setFilter(filter); // 실행할 필터 객체를 등록
        //registration.addUrlPatterns("login.me"); // 어디서 사용할지 결정 가능
        registration.addUrlPatterns("/*");    // 모든 url 패턴에 대해서 필터를 적용하겠다.
        registration.setOrder(1);           // 필터 우선 순위 -> 숫자가 낮을수록 먼저 실행한다.
