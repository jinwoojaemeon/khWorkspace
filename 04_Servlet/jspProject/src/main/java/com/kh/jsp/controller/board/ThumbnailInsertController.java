...
public class ThumbnailInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if(JakartaServletFileUpload.isMultipartContent(request)) {
			...
			Board b = new Board();
			ArrayList<Attachment> attachmentList = new ArrayList<>(); // 여러 파일을 담을 리스트
			...
			for(FileItem item : formItems) {
				...
				if(item.isFormField()) {//일반파라미터
					...
					if(originName.length() > 0) { //파일업로드가 실제로 되었을 때
						...
						// 파일 필드명에 따라 썸네일과 상세이미지 구분
						if("file1".equals(item.getFieldName())) {
							at.setFileLevel(1); // 썸네일 (대표이미지)
						} else {
							at.setFileLevel(2); // 상세이미지
						}
						
						attachmentList.add(at); // 리스트에 추가
					}
				}
			}
			
			// b -> 게시글 정보
			// attachmentList -> 첨부파일 리스트 (썸네일 + 상세이미지들)
			
			int result = new BoardService().insertThumbnailBoard(b, attachmentList);
