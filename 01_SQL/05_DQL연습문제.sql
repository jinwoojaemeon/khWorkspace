...
SELECT EMP_NAME, SALARY*12 AS 연봉, 
        (SALARY + (SALARY * NVL(BONUS,0)))*12 AS 실수령액, 
...
FROM EMPLOYEE 
JOIN SAL_GRADE ON(SALARY BETWEEN MIN_SAL AND MAX_SAL)    -- 어떻게 조인할지에 대한 조건식?
WHERE SAL_LEVEL = 'S1';
...
SELECT *
FROM EMPLOYEE
WHERE MOD(EMP_ID, 2) = 1;    -- EMP_ID > VARCHAR -> 형 변환 진행

--19. EMPLOYEE 테이블에서 근무 년수가 20년 이상인 직원 정보 조회
SELECT *
FROM EMPLOYEE
-- WHERE EXTRACT(YEAR FROM SYSDATE)- EXTRACT(YEAR FROM HIRE_DATE) >= 20;
-- WHERE ADD_MONTHS(HIRE)DAT, 240) < SYSDATE 
...
FROM EMPLOYEE
WHERE TO_NUMBER(SUBSTR(EMP_NO, 3, 2)) BETWEEN 1 AND 12  -- 월 1~12
