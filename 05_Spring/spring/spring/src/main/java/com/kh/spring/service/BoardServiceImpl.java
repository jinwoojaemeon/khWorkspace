...
public class BoardServiceImpl implements BoardService {
    ...
    public Map<String, Object> getBoardList(int currentPage) {
        ...
        List<Board> list = boardMapper.selectBoardList(rowBounds);
    ...
    @Override
    public Map<String, Object> getSearchBoardList(int currentPage, String condition, String keyword) {
        int listCount = boardMapper.selectSearchBoardCount(condition, keyword);
        ...
        List<Board> list = boardMapper.selectSearchBoard(rowBounds, condition, keyword);
    ...
    public int insertBoard(Board board, MultipartFile file) {
        ...
        if(result > 0 && file != null && !file.isEmpty()) {
            ...
            at.setChangeName(saveFile(file, "C:/workspace/05_Spring/spring/spring/src/main/webapp/resources/uploadFiles/"));
            at.setFileLevel(1);
    ...
    @Override
    public List<Board> getThumbnailList() {
        return boardMapper.selectThumbnailList();
    }

    @Override
    public Map<String, Object> getThumbnailBoardDetail(int boardNo) {
        int result = boardMapper.increaseCount(boardNo);

        Map<String, Object> map = new HashMap<>();
        if(result > 0) {
            Board board = boardMapper.selectThumbnailBoardByBoardNo(boardNo);
            List<Attachment> attachmentList = boardMapper.selectAttachmentList(boardNo);
    ...
    @Override
    @Transactional
    public int insertThumbnailBoard(Board board, List<MultipartFile> attachmentList) {
        int result = boardMapper.insertThumbnailBoard(board);

        if(result > 0 && attachmentList != null && !attachmentList.isEmpty()) {
