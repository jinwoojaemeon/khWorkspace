...
-- 표시하는 SQL문장을 작성 (단, 헤더는 '학번', '이름', '입학년도'가 표시되도록 한다.)
...
/*
춘 기술대학교의 남자 교수들의 이름과 나이를 출력하는 SQL문장을 작성하시오.
단, 이때 나이가 적은 사람에서 많은 사람 순서로 화면에 출력되게 만드시오.
...
FROM TB_STUDENT
WHERE FLOOR((TO_CHAR(ENTRANCE_DATE - TO_DATE('19'||SUBSTR(STUDENT_SSN,1,6),
                                                'YYYYMMDD'))) / 365) > 19;
...
SELECT TO_CHAR(TO_DATE('20/10/11','YY/MM/DD'), 'YY"년" MM"월" DD"일"'),
        TO_CHAR(TO_DATE('49/10/11','YY/MM/DD'), 'YY"년" MM"월" DD"일"'),
        TO_CHAR(TO_DATE('20/10/11','YY/MM/DD'), 'RR"년" MM"월" DD"일"'),
        TO_CHAR(TO_DATE('49/10/11','YY/MM/DD'), 'RR"년" MM"월" DD"일"')
FROM TB_STUDENT;
...
/*9. 학번이 A517178 인 한아름 학생의 학점 총 평점을 구하는 SQL문을 작성하시오.
     ...
     점수는 반올림하여 소숫점 이하 한자리까지만 표시한다 */
SELECT ROUND(AVG(POINT),1) AS "평점"
FROM TB_GRADE
WHERE STUDENT_NO = 'A517178';

/* 10. 학과별 학생 수를 구하여 "학과번호", "학생 수(명)"의 형태로 헤더를 만들어 
        결과값이 출력되도록 하시오. */
...
/*12. 학번이 A112113인 김고운 학생의 년도 별 평점을 구하는 SQL 문을 작성하시오.
      ...
      점수는 반올림하여 소숫점 이하 한 자리까지만 표시된다. */
SELECT DISTINCT SUBSTR(TERM_NO,1,4) AS "년도", ROUND(AVG(POINT),1) AS "년도 별 평점"
FROM TB_GRADE
WHERE STUDENT_NO = 'A112113'
GROUP BY SUBSTR(TERM_NO, 1, 4);

/* 13. 학과 별 휴학생 수를 파악하고자 한다. 학과 번호와 휴학생 수를 표시하는 SQL문을 작성하시오 */
