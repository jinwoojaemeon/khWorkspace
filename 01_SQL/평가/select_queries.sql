...
*/
SELECT TITLE, TEACHER_NAME, STATUS
FROM ENROLLMENT
...
*/
SELECT TITLE, ROUND(AVG(GREADE_SCORE),1)
FROM LECTURE
JOIN ENROLLMENT USING(LECTURE_ID)
GROUP BY TITLE
HAVING AVG(GREADE_SCORE) >= 80
ORDER BY AVG(GREADE_SCORE) DESC;
...
*/
DROP TRIGGER UPDATE_LEC_CNT;
CREATE OR REPLACE TRIGGER UPDATE_LEC_CNT
AFTER INSERT
ON ENROLLMENT
FOR EACH ROW
BEGIN
